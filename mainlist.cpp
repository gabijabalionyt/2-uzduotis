            ...
            else
            {
                ...
                else
                {
                    ...
                    for (list<Student>::iterator IT = Failed.begin(); IT != Failed.end(); IT++)
                    {
                        ...
                        if (AverageChoice==1)
                            WriteToFailed << std::fixed << setprecision(2) << FinalMark ((*IT), Average)  << setprecision(prec) << endl;
                        else
                            WriteToFailed << std::fixed << setprecision(2) << FinalMark ((*IT), Median)   << setprecision(prec) << endl;
                    ...
                    for (list<Student>::iterator IT = Passed.begin(); IT != Passed.end(); IT++)
                    {
                        ...
                        streamsize prec = cout.precision();
                        if (AverageChoice==1)
                            WriteToPassed << std::fixed << setprecision(2) << FinalMark ((*IT), Average)  << setprecision(prec) << endl;
                        else
                            WriteToPassed << std::fixed << setprecision(2) << FinalMark ((*IT), Median)   << setprecision(prec) << endl;
                    }
                    for (auto i = 0; i != LongestName + LongestSurname + 10; i++)
                        WriteToPassed << endl;
                    WriteToPassed.close();
